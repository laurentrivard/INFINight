<?php
// This is the server API for the INFINight iPhone app. To use the API, the app
...
// add new event to the database
 ...
 //adds user to active_users database
	function handleJoin()
	{
		...
		$matricule = $this->getString('matricule', 255);
		$groupe = $this->getString('groupe', 11); //make sure i am getting an int
		$grad_year = $this->getString('grad_year', 11 ); //make sure this is also an int
...
//		$stmt->execute(array($device_id));
		...
		$stmt = $this->pdo->prepare('INSERT INTO active_users (device_id, device_token, name, matricule, groupe, grad_year) VALUES (?, ?, ?, ?, ?, ?)');
		$stmt->execute(array($device_id, $device_token, $name, $matricule, $groupe, $grad_year));
...
//	}
	...
	{
	echo 'in handle message';
//		$udid = $this->getUDID();
//		$text = $this->getString('text', self::MAX_MESSAGE_LENGTH, true);
	
	$message = $this->getString('text', 150);
...
//			$payload = $this->makePayload($user->nickname, $text);
			...
			$payload = $this->makePayload($message, '');
...
//			$stmt->execute(array($user->secret_code, $user->device_token));
			$stmt = $this->pdo->prepare("SELECT device_token FROM active_users");
			$stmt->execute();
